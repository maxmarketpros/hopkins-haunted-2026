...
- **Prefetch payload aliases.** The static export writes segment prefetch payloads as `<route>/__next.<segment>/__PAGE__.txt` while the client requests `<route>/__next.<segment>.__PAGE__.txt`. `scripts/postbuild.mjs` (run by `npm run build`) writes the flat-named copies so client-side navigation prefetches never 404.
- **Phones skip the hero video.** Under 768px the poster stays; the 3 MB loop only loads on desktop after the page `load` event. The poster is a real `<img fetchpriority="high">` beneath the video so it is the LCP element.
